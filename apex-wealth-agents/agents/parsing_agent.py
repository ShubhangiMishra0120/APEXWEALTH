...
class ParsingAgent:
    ...
    def parse_query(self, user_query: str, context: List[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        Returns:
            Dict with keys:
            - query_type: str (transaction_analysis, investment_advice, market_question, general_knowledge, etc.)
            - intent: str (brief description)
            - requires_knowledge: bool (needs VectorDB retrieval)
            - requires_transaction_data: bool (needs CSV/DB data)
            - requires_market_data: bool (needs real-time market data)
            - keywords: List[str] (keywords for VectorDB search)
            - risk_profile_needed: bool
        """
        context_str = ""
        if context:
