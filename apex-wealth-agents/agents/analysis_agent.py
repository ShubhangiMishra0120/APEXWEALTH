...
class AnalysisAgent:
    ...
    def extract_financial_data_from_transactions(
        ...
        # If no income in profile, estimate from transaction data
        if income == 0 and transaction_summary:
            # Estimate: assume income is 1.5x monthly expenses (rough estimate)
            monthly_spend = transaction_summary.get("monthly_spend", 0)
            if monthly_spend > 0:
                income = monthly_spend * 1.5
